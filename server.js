...
app.get("/api/offers/:id/offer-letter", (req, res) => {
  ...
  db.get("SELECT * FROM onboarding WHERE id=?", [id], (err, row) => {
    if (!row) return res.send("Offer not found");
...
(offer_id,candidate_name,email,mobile,designation,salary,work_location,date_of_joining,status)
VALUES (?,?,?,?,?,?,?,?,?)
`;
  ...
  db.run(
    ...
    function (err) {
      ...
      const link = `https://hr-frontend-bay.vercel.app/onboarding/${offer_id}`;
...
app.post(
  ...
  (req, res) => {
    ...
    db.run(
      "UPDATE onboarding SET status='Completed' WHERE offer_id=?",
      [offer_id],
      function (err) {
        if (err) return res.json({ success: false });
